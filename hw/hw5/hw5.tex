...
\begin{enumerate}
    \item Explanation of algorithm:
    
    \begin{itemize}
        \item Order all quests by $g_i$
        \item Initialize an empty max heap
        \item Loop through $1...k$. At the $i^{th}$ step, insert all quests with $f_i \le i$ into the max heap. Pop the max as the $i^{th}$ choice. 
    \end{itemize}

    \-\
    \item Runtime analysis:
    \begin {itemize}
        \item Sorting elements takes $O(n\cdot log(n))$
        \item Inserting and removing from the max-heap takes $O(logn)$
        \item Each quest is inserted into the heap
    \end {itemize}
...
\includegraphics[width=0.65\textwidth]{src/01.png}
...
\begin{itemize}
    \item Identify all negative edges $E' = \{ e \in E | w(e) < 0 \}$
    \item Identify connected components from those edges via DFS. Label a component that contains any vertex $u$ as $C(u)$ 
    \item let $H$ be a subgraph of $G$: $H = \{ V, (u, v) \in E \backslash E' | C(u) \ne C(v)\}$
    \item Now the graph $H$ is a standard, positive edged connected graph, which can be a candidate of any MST algorithm.
\end{itemize}

The overall time analysis is $O(|E| \cdot log(|V|))$ or $O(|V|^2)$ depending on the implementation used for the MST
\begin{itemize}
    \item Constructing $E' \in O(|E|)$
    \item Decomposition algorithm to get SCCs $\in O(|E| + |V|)$
    \item Constructing $H \in O(|E|)$
    \item Running MST via Prims algorithm $\in O(|V|^2)$ (Array implementation of Priority Queue) or $\in O(|E| \cdot log(|V|))$ (min-heap implementation of priority Queue)
\end{itemize}
...
\-\ \newline
\includegraphics[width=0.65\textwidth]{src/02.png}

The above graph represents the number of colors needed for a given graph plotted against the number of verticices in that graph. Specifically,
\begin{itemize}
    \item x-axis: number of verticies, $|V|$, in the graph. The experiment was run on inputs of $2^k, k \in \{ 3,...12\}$
    \item y-axis: number of colors needed to properly solve the graph coloring problem. For each input, 5 graphs were generated. Above is the average of the number of colors needed for each. 
    \item Color: The color denotes how the verticies were ordered before applying the greedy solution to the problem. Red ordered verticies with the most neighbors first, blue did the opposite. 
\end{itemize}

Questions:

\begin{enumerate}
    \item Generally the number of colors needed to color the graphs increases with $|V|$, however the increase is only marginal. The correlation is roughly $0.1$
    \item Different graphs of the same size generally had similar number of colors. The largest ever recorded difference was $\pm 1$ (for 1 input, 1 graph needed 4 colors, another 5, and another 6.)
\end{enumerate}
