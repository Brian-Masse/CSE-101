...
\item Sub-problems:
\begin{itemize}
    \item Each subproblem represents the next hotels we can stop at after our current hotel. 
    \item Each subproblem has some $a_i,...a_n$ and returns the lowest cost path from $i$ to $n$. Each subproblem contains the same set of hotels.
...
\item Base Case: 
\begin{itemize}
\item The base case is when  $i==n$, i.e. the subproblem where the start position is the final hotel. $TravelPenalty[n] = 0$
\end{itemize}
\item Recursive definition:
\begin{itemize}
    \item If we are not at our final destination (the base case), our next hotel stop after $a_i$ is some $a_j$. Because we can only travel forwards, we know $i < j \leq n$. If we stop at $a_j$, the penalty for this travel day is stored in  $TravelPenalty[j] = (200 - (a_i-a_j))^2$. To find the penalty for $i$: $TravelPenalty[i] = min_{i+1\leq j<n}(200 - (a_i-a_j))^2 + TravelPenalty[j]$.
\end{itemize}
...
\item Form of output (how do we get the final answer?):
\begin{itemize}
    \item The final answer is given by $TravelPenalty[0]$ as we fill the array in reverse order, so this will be the minimum penalty to travel $n$ miles to the final destination $a_n$.  
\end{itemize}
...
\item ~~~~~$MinPenalty= min(MinPenalty, (200-a_I+a_J)^2 + TravelPenalty[J])$.
...
\item Runtime analysis:
\begin{itemize}
    \item There are two nested for loops, so the runtime is quadratic $O(n^2)$.
\end{itemize}
\item Small example:
