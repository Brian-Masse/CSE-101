...
Thus, as a function of m, an upperbound for the number of recursive calls is given by $m!$
...
\begin{enumerate}
    ...
    The space complexity is $O(n)$: There is a constant amount of data stored for each node. 
...
\item Sub-problems:
\begin{itemize}
    \item Each subproblem represents finding the lowest-penalty path from the current hotel to the final hotel, $a_n$.
    \item Each subproblem is given a suffix of the array of hotel positions, $a_i,...a_n | 1 \le i \le n$. This means there are $n$ distinct subproblems.
    \item The solutions to each subproblem can be stored in an array, $TravelPenalty[]$, of length $n$, where $TravelPenalty[i] = $ the minimum penalty from the hotel at $a_i$ to the final hotel.
...
~~~~$TravelPenalty[i] = min_{i+1\le j \le n}\big( (200 - (a_j-a_i))^2 + TravelPenalty[j] \big)$.

\-\ \newline
Brief Explanation of Correctness:

\begin{itemize}
    \item If we choose to stop at some future hotel at $a_j$, $j > i$, the distance driven for that day is $x = a_j - a_i$. Thus the penalty for that day is $(200 - (a_j - a_i))^2$
    \item The minimized penalty to get from the hotel at $a_j$ to the hotel at $a_n$ is already stored in $TravelPenalty[j]$
    \item Thus, the total penalty of going from the hotel at $a_i$ to $a_n$ is $\big( (200 - (a_j-a_i))^2 + TravelPenalty[j] \big)$
    \item Then, by minimizing this total penalty, we correctly find the smallest penalty to get form $i$ to $n$
\end{itemize}

\-\
\item Order in which sub-problems are solved:
\begin{itemize}
    \item Because we can only travel forwards, we need to fill the array in reverse order i.e. starting at $ i =n-1$ to $i=1$. 
\end{itemize}

\-\
\item Form of output:

The final answer is given by $TravelPenalty[0]$, the minimized penalty of going from mile post 0 to mile post $n$.

\-\ \newline
\emph{To find the chain of hotels you must stop at, TraversalPenalty can be trivially modified to also store the next best hotel at each location. }

\-\
\item Pseudocode:
\begin{itemize}
\item $a_0=0$.
\item Initialize $TravelPenalty[0..n]$.
\item $TravelPenalty[n]=0$
\item FOR $I=n-1$ down to $1$ do:
\item ~~~$MinPenalty = \infty$
\item ~~~FOR $J=I+1$ to $n$ do:
\item ~~~~~$MinPenalty= min(MinPenalty, (200-a_J+a_I)^2 + TravelPenalty[J])$.
\item~~~~$TravelPenalty[I]=MinPenalty$
\item Return $TravelPenalty[0]$.
\end{itemize}
...
\begin{itemize}
    \item $TravelPen[4] = 0$
    \item $TravelPen[3] = (200-(500-400))^2 + TravelPen[4] = 10000 + 0 = 10000$
    \item $TravelPen[2] = min( (200-25)^2+ TravelPen[3] , (200-(500-150))^2 +TravelPen[4]) = min(30625+10000, 22500) = 22500$
    \item $TravelPen[1] = min((200-(150-25))^2 + TravelPen[2] + TravelPen[3]), (200-(500-25))^2) + TravelPen[4] = min(5625+22500+10000, 75625) = 38125$
    \item $TravelPen[0] = min((200-25)^2 + TravelPen[1], (200-150)^2+TravelPen[2], (200-400)^2+TravelPen[3], (200-500)^2 + TravelPen[4] = min(68750,25000,50000,90000)) = 25000$.
The route with the lowest penalty is to stop at hotel 2 (mile 150) and hotel 3 (mile 400). The penalty for the first travel day is 1500, and the penalty for the second travel day is 22500.
...
\begin{enumerate}
    \item Characterize Subproblem
    
    Let $I_1,..I_n$ be the set of objects ordered by non-increasing value to cost ratio: $v_i / c_i$

    \-\ \newline
    Each subproblem is given some suffix of this array with the budget leftover after attempting to maximize the selection of all higher valued $I_1, I_2, ...$. For example, subproblem $j$ is given the set of object $(I_j,..I_n)$ and budget $U - \sum_{1 \le i<j} p_i\cdot c_i $

    \-\ \newline
    Then, the set of subproblems is all subsets $u_j,...u_n | 1 \le j \le n$ and the empty set. Therefore the set of all unique subproblems, $S$, $\implies |S| = n + 1$

    \-\
    \item Base Case
    
    $U = 0 \implies p_j,...p_n = 0$ (all remaining items have frequency 0)
    ...
    \begin{eqnarray*}
        P[i] &=& \lfloor U'[i - 1] / c_i \rfloor \\
        V[i] &=& V[i - 1] + P[i] \cdot v_i \\
        U'[i] &=& U'[i - 1] - P[i] \cdot c_i
    \end{eqnarray*}
    ...
    \begin{itemize}
        \item $p_j$ is just the most of item $I_j$ we can currently afford with the leftover budget ($U'[j - 1]$)
        \item the total value after picking all items $I_j$ is the previous value $(V[j -1])$ + the new value from item j ($p_j * v_j$)
        \item The new leftover budget is the old leftover budget $(U'[j-1])$ - the cost of item j $(p_j * c_j)$
    \end{itemize}
    ...
    Either return $V[n]$ for the maximized value, or return $P$ for the total amounts of each item. 
    ...
    \item Time Analysis
    \begin{itemize}
        \item Sorting $\in O(n\cdot logn)$
        \item Loop: $\in O(n) \cdot O(1) \in O(n)$
    \end{itemize}

    Therefore, the total runtime is $O(n \cdot logn)$

    The space complexity is $O(3n) \in O(n)$: There is a constant amount of data stored for each of the $n$ items.

    \-\ 
    \item Small Example
    
    \includegraphics[width=0.65\textwidth]{src/Q4J.png}
