...
\textbf{Balanced BST / multiset (recommended for these ops):}
insert $= O(\log k)$,\; delete $= O(\log k)$,\; get\_max $= O(1)$ if we keep a pointer to the largest (or $O(\log k)$ to query the max).

\textbf{Plain double-ended queue (without monotone maintenance):}
push/pop at ends $= O(1)$, but get\_max requires scanning $= O(k)$.
