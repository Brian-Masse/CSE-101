...
Given an undirected graph G, we can convert each undirected edge to two directed edges. If the graph $G$ is directed, we skip this step. Now with our directed graph we can create an adapted graph (G2) where, for each vertex $u$ in $G$, we can add the vertex weight $u_\omega$ to the edge weight $\omega(u,v)$ for all outgoing edges from vertex $u$. With this adapted graph, we can run Dijkstra's graph search algorithm to find the shortest weighted path from $u$ to $v$. 
\\\\
Proof of correctness: For this problem, we only need to create the new edge weights for graph G2 from G, and then can run the graph search algorithm Dijkstra's from class, and so the proof of correctness to find the shortest path is the same. 
\\\\
Time analysis and efficiency: To implement the graph, we will use the priority queue implementation. In our adapted graph (G2), the number of vertices does not change from the original graph (G). If graph G was originally undirected, the number of edges in G2 is doubled so the runtime of the algorithm is $O(deletemin * |V|+ decreasekey*2|E|)$. If the original graph was directed, the number of vertices and the number of edges in G2 is the same as G, so the runtime is $O(deletemin * |V|+ decreasekey*|E|)$ using a priority queue. 
