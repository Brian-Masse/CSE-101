...
The window set $WS[I+1]$ differs from $WS[I]$ by one element leaving and one new element entering the window. Specifically, $(I, A[I])$ is removed from the set and $(I+k+1, A[I+k+1])$ is added, given that ($I+k+1 \le n$).

\[
WS[I{+}1] \;=\; \big( WS[I] \setminus \{(I, A[I])\} \big)\; \cup\;
\begin{cases}
\{(I{+}k{+}1, A[I{+}k{+}1])\}, & \text{if } I{+}k{+}1 \le n,\\
...
\begin{itemize}
  ...
  \item get\_max() - To get the current maximum value within the window
\end{itemize}
\-\
...
\begin{itemize}
    \item Balanced Binary Search Trees (BST or RST)
...
% PART E
\item What is the maximum size of this data structure?

At most $k{+}1$ elements (the window is $[I,\,\min(I{+}k, n)]$ inclusive).
\-\ \newline

% PART F
\item How long do the different data structure operations take?

\emph{BST:}
\begin{itemize}
    \item insert $= O(\log k)$
    \item delete $= O(\log k)$
    \item get\_max $= O(1)$ if we keep a pointer to the largest (or $O(\log k)$ to query the max).
\end{itemize}
\-\

\emph{Plain Dequeue (without monotone maintenance):}
\begin{itemize}
    \item push/pop at ends $= O(1)$
    \item get\_max requires scanning $= O(k)$
\end{itemize}
\-\
...
\begin{itemize}
    \item get\_max \(= O(1)\)
    \item insert $= O(\log k)$ (\(O(k)\) in worst case)
    \item delete $= O(\log k)$ (\(O(k)\) in worst case)
\end{itemize}
\-\ 

\item Give pseudo-code for an algorithm to solve this problem using data structure operations.
...
\item Give a time analysis for your algorithm, using answers to previous questions. 
\begin {itemize}
\item The loop runs for each index of the array. 
\item The heap is initialized in \(O(logk)\)
\item heap.peek runs in \(O(1)\)
\item heap.remove / heap.insert both run in \(O(logk)\) in the average case. 
\end {itemize}

\begin{eqnarray*}
    \textnormal{Time Complexity} &\in& O(logk + n(2\cdot logk))\\
    &\in& O(n\cdot logk)
\end{eqnarray*}
...
\-\ \newline
\emph{Time Analysis and Efficiency}

The modification uses the same loop / logic as the base djikstras algorithm, and thus shares time complexity.

Specifically, each node \(|V|\) is removed from the heap (removeMin) once. And each edge is checked \(|E|\) once.
The priorityQueue is implemented with a mean heap, which has the following properties:
\begin{itemize}
    \item removeMin: \(= O(logn)\)
    \item decreaseKey: \(= O(logn)\)
\end{itemize}
...
The proof of correctness is largely the same as the proof of correctness for the base djikstras algorithm, since loop, data structure, and operations are the same. 
Vertex weights can be thought of as being added onto all incoming verticies. 

Specifically, for correctness, use the loop invariant: \emph{When a node u is removed from the priorityQueue, dist[u] is the shortest path, including node weights, from s to u.}

\begin{itemize}
    \item The base case, when s is removed, is trivially correct. Since the shortest path from s to itself is \(weight[s]\)
    \item For the general case, assume that a group \(u_1,..u_k\) have been removed and that \(dist[u_i]\) is the shortest path from s to \(u_i\)
    We must show that when \(u_{k+1}\) is removed, that \(dist[u_{k_+1}]\) is the shortest path.

    The remained of the proof is the same as we discussed in class: any supposedly shorter path from s to \(u_{k+1}\) would need to cross form the group \(u_1...u_k\) to outside the explored nodes. 
    However, any such path would necessarily include a positive edge and vertex weight, making it longer. 
    If those edges / verticies have weight 0, then at best we would have found a path of equivalent distance, proving the invariant. 
